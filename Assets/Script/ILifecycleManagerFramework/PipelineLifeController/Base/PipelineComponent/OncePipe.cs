...
{
    public class OncePipe<T> : ALifePipelineComponent<OncePipe<T>> where T : ILifecycleSubscriber
    {
        ...
        public override void Execute(LifecyclePipelineManager.LifecyclePipelineContext context)
        {
            ...
            foreach (var item in list)
            {
                ...
                {
                    Debug.Log($" <{item.GetType().FullName}> 尝试被注册进入 OncePipe ,但是其没有实现 {typeof(T)} 接口");
                }
